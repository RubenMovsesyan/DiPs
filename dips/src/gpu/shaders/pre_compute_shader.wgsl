    ...
    for (var i = -win_size_2; i < win_size_2; i++) {
        for (var j = -win_size_2; j < win_size_2; j++) {
            ...
            if (i32(coords.x) + i >= i32(dimensions.x) || i32(coords.y) + j >= i32(dimensions.y) || i32(coords.x) + i < 0 || i32(coords.y) + j < 0) {
                ...
                color = textureLoad(input_texture, vec2<u32>(u32(i32(coords.x) + i), u32(i32(coords.y) + j))).b;
            }
...
fn pre_compute_main(
    ...
    start_median_array[0] = spatial_median_filter(coords.xy, dimensions.xy, start_texture_array[0]).b;
    start_median_array[1] = spatial_median_filter(coords.xy, dimensions.xy, start_texture_array[1]).b;
    start_median_array[2] = spatial_median_filter(coords.xy, dimensions.xy, start_texture_array[2]).b;
    start_median_array[3] = spatial_median_filter(coords.xy, dimensions.xy, start_texture_array[3]).b;
