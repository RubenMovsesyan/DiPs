...
impl PreComputeBindGroupsContainer {
    pub fn initialized(&self) -> bool {
        if let Some(_) = self.texture_dimensions {
            true
        } else {
            false
        }
    ...
    pub fn new(device: &Device) -> Self {
        ...
        Self {
            ...
            start_textures_bind_group_layout,
            start_textures_bind_group: None,
            start_textures: Vec::with_capacity(TEMPORAL_BUFFER_SIZE),
    ...
    // WARN this should only be called when the size of textues
    // is TEMPORAL_BUFFERS_SIZE
